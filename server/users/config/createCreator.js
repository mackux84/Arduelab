...
module.exports = {
  ...
  validate: {
    ...
    payload: Joi.object({
      ...
      telephone: Joi.string().regex(/^(\(?\+[0-9]{1,3}\)?)? ?-?\(?[0-9]{1,3}\)? ?-?[0-9]{3,5} ?-?[0-9]{4}( ?-?[0-9]{3})? ?(\w{1,10}\s?\d{1,6})?/).required().description('Telefono del creador de experimento').example('800 555 1212'),
      cellphone: Joi.string().regex(/^(\(?\+[0-9]{1,3}\)?)? ?-?\(?[0-9]{1,3}\)? ?-?[0-9]{3,5} ?-?[0-9]{4}( ?-?[0-9]{3})? ?(\w{1,10}\s?\d{1,6})?/).required().description('Celular del creador de experimento').example('(311) 555-1212'),
    })
  },
